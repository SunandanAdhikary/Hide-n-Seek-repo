...
int main()
{
    ...
    while(l<=10)
    {
        ...
        // Sort the attack window of each instance of each target
        for(i=0; i<ECUCount; i++)
        {
            InsSortByAtkWinLen(&candidates[i].instances,0,candidates[i].count-1);
            // printing the status of each candidate before applying obfuscation strategies
            printf("\n Canditate ID=%d",candidates[i].ID);
            printf("\n--------------------------------------------------");
            for(j=0;j<candidates[i].count;j++)
            {
                printf("\n %d: Instance=%d: attack win len=%d, attack win count=%d",j, candidates[i].instances[j].index, candidates[i].instances[j].atkWinLen, candidates[i].instances[j].atkWinCount);
                printf("\n Attack window:");
                for(k=0;k<candidates[i].instances[j].atkWinCount;k++)
                    printf("%d(instance=%d)  ",candidates[i].instances[j].atkWin[k],candidates[i].instances[j].insWin[k]);
            }
            printf("\n Pattern: ");
            for(j=0;j<candidates[i].count;j++)
        ...
        for(i=0;i<ECUCount;i++)
        {
            ...
            else // Checking obfuscation 2
            {
                ...
                for(j=0;j<i;j++) //*** change this code
                {
                    // Checking if higher priority task belongs to attack window of target instance
                 ...
                 { // Checking obfuscation 3
                    ...
                    {
                        k++;
                        if(candidates[i].periodicity == candidates[k].periodicity && CheckMembership(candidates[i].instances[insToSkipObf1].atkWin,candidates[i].instances[insToSkipObf1].atkWinCount,candidates[k].ID>=0))
                        {
